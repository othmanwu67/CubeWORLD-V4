...
var passcode = "";
var err = false;
var ignorehtml = ["b","i","u","s","font size=5","div","div class='quote'","h5"];
//Bot API client side!
class bot{
...
function errorFatal() {
    ("none" != $("#page_kick").css("display") && "none" != $("#page_kick").css("display")) || $("#page_kick").show();
}
function setup() {
    $("#chat_send").click(sendInput),
      ...
      socket.on("miss", function (a) {
          var b = bonzis[a.guid];
          b.runSingleEvent([{type:"anim",anim:"sad_fwd",ticks:20},{type:"text",text:"I miss you, "+a.user},{type:"idle"}])
      }),
      ...
      socket.on("sad", function (a) {
          var b = bonzis[a.guid];
          b.cancel(), b.sad(a.verysad);
      }),
        socket.on("asshole", function (a) {
            var b = bonzis[a.guid];
            b.cancel(), b.asshole(a.target);
        }),
      socket.on("faggot", function (a) {
          var b = bonzis[a.guid];
          b.cancel(), b.faggot(a.target);
      }),
                                ...
                                items:{
                                  ...
                                  bless:{
                                   ...
                                   niggle:{
                                        ...
                                        name:"Jewify✡ (Ϫ Only.)",
                                        callback:function(){
                                            socket.emit("command",{list:["faggy", d.id]});
                                        }
                                    },
                                  rabbi:{
                                        name:"rabbi (∃ Only.)",
                                        callback:function(){
                                            socket.emit("command",{list:["kingger", d.id]});
                                        }
                                    },
                            }
...
callback: function () {
     ...
     callback: function () {
      ...
      faggot: {  
        name: "Call an faggot",
      callback: function () {
            socket.emit("command", { list: ["faggot", d.userPublic.name] });
        },
      },
      gofag: {  
        name: "Call an Go!fag",
...
name:"Pastule",
callback:function(){
socket.emit("talk",{text: d.userPublic.name+" stop being a kiddie or i will fuck you"});
},
},
                                nigger:{
                                    name:"Niggerify",
                ...
                },
                {
                    ...
                    value: function () {
                       //OK solution as long as no real color starts with "http"
                        if(this.color.startsWith("https://")){
                        ...
                        if(this.color == "god" && this.offtag == undefined){
                            $("#bonzi_"+this.id).append("<div id='tag_"+this.id+"' class='official_tag'><b><i>Owner Of The CubeWORLD</i></b></div>")
                            this.offtag = $("#tag_"+this.id);
                        } else if(this.color == "rabbigem" && this.offtag == undefined){
                            $("#bonzi_"+this.id).append("<div id='tag_"+this.id+"' class='official_tag'><b><i>rabbi</i></b></div>")
                            this.offtag = $("#tag_"+this.id);
                      ...
                      this.offtag = $("#tag_"+this.id);
                        } else if(this.color == "dirty" && this.offtag == undefined){
                      $("#bonzi_"+this.id).append("<div id='tag_"+this.id+"' class='official_tag'><b><i>DIRTY NIGGER</i></b></div>")
                      this.offtag = $("#tag_"+this.id);
                        } else if(this.userPublic.bot && this.offtag == undefined){
                            $("#bonzi_"+this.id).append("<div id='tag_"+this.id+"' class='official_tag'><b><i>BOT</i></b></div>")
                            this.offtag = $("#tag_"+this.id);
                              } else if(this.color != "god" && this.color != "dirty" && this.color != "rabbigem" && 
                                this.color != "king" && !this.userPublic.bot && this.offtag != undefined){
                            this.offtag.remove();
...
document.getElementById("logcontent").innerHTML += "<p><font color='"+this.userPublic.color+"'>"+this.userPublic.name+": </font>"+a+"</p>",                      document.getElementById("logcontent").scrollTop = document.getElementById("logcontent").scrollHeight,
              ...
              key: "sad",
                  value: function (a) {
                      ...
                      a && (b.push({ type: "anim", anim: "sad_fwd", ticks: 30 }), b.push({ type: "idle" })), this.runSingleEvent(b);
                  },
              },
                {
                    key: "updateDialog",
                ...
                },
                {
                    key: "faggot",
                    value: function (a) {
                        this.runSingleEvent([{ type: "text", text: "Hey, " + a + "!" }, { type: "text", text: "You're a fucking faggot!", say: "your a fucking faggot!" }, { type: "anim", anim: "grin_fwd", ticks: 15 }, { type: "idle" }]);
                    },
                },
              {
                  key: "gofag",
                  value: function (a) {
                      this.runSingleEvent([{ type: "text", text: "Hey, " + a + "!" }, { type: "text", text: "You're a fucking go!fag!", say: "your a fucking gofag!" }, { type: "anim", anim: "grin_fwd", ticks: 15 }, { type: "idle" }]);
                  },
                },
              {
                  key: "asshole",
              ...
              },
                {
                    ...
                    value: function (a) {
                        this.runSingleEvent([
                            { type: "text", text: "*notices " + a + "'s CubeBulge™*", say: "notices " + a + "s cubebulge" },
                            { type: "text", text: "owo, wat dis niggers? i hope you will kys", say: "oh woah, what diss niggers? i hope you will kill yourself" },
                        ]);
                    },
                },
                {
    ...
    BonziData = {
        ...
        event_list_joke_open: [
            ...
            [{ type: "text", text: "{NAME} used /joke. Whoop-dee-Whoop-dee-Whoop-dee-fucking doo." }],
            [{ type: "text", text: "HEY YOU FAGGOTS ITS TIME FOR A JOKE" }],
            [
                { type: "text", text: "Wanna hear a joke?" },
                { type: "text", text: "No?" },
                { type: "text", text: "Mute me then. That's your fucking problem." },
            ],
            [{ type: "text", text: "My {NAME}'s sister wants me to tell a joke." }],
        ...
        event_list_joke_mid: [
            ...
            [
                ...
                { type: "text", text: "WAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA!" },
            ...
            [
                {
                    type: "text",
                    text: 'Two sausages are in a pan. One looks at the other and says "Boy it\'s hot in here!" and the other sausage says "OMG! It\'s a talking sausage!"',
                    say: "Two sausages are in a pan. One looks at the other and says, Boy it's hot in here! and the other sausage says, Oh my god! It's a talking sausage!",
                },
                { type: "anim", anim: "shrug_back", ticks: 15 },
                { type: "text", text: "What were you expecting? A fuck joke? You're a sick shit." },
            ],
            [
                { type: "text", text: "What is in the middle of Paris?" },
                { type: "text", text: "A giant inflatable assplug." },
            ],
            [
                { type: "text", text: "What goes in pink and comes out blue?" },
                { type: "text", text: "Sonic's faggot." },
            ],
            [
                { type: "text", text: "What type of water won't freeze?" },
                { type: "text", text: "Your far's." },
            ],
            [
            ...
            [
                ...
                { type: "text", text: "Piss in them." },
            ],
            [
                { type: "text", text: "Why do we call money bread?" },
                { type: "text", text: "Because we KNEAD it. LMAO please send money to my PayPal at nigerianprince99@bonzi.com" },
            ],
            [
                { type: "text", text: "What is a cow that eats grass?" },
                { type: "text", text: "ASS" },
                { type: "text", text: "I'm a fucking comedic genius, I know." },
            ],
        ],
        event_list_joke_end: [
            [
                { type: "text", text: "You know {NAME}, a good friend laughs at your jokes even when they're not so funny." },
                { type: "text", text: "And you fucking asshole. Thanks." },
            ],
            [{ type: "text", text: "Where do I come up with these? My ass?" }],
            [
                { type: "text", text: "Do I amuse you, nigga {NAME}? Am I funny? Do I make you laugh?" },
                { type: "text", text: "pls respond or i will kill u", say: "please respond or i will kill you" },
            ],
        ...
        event_list_fact_mid: [
            [
                ...
                { type: "text", text: "Did you know that you can run GDI malwares on linux using wine?" },
                { type: "anim", anim: "earth_back", ticks: 15 },
                { type: "anim", anim: "grin_fwd", ticks: 15 },
            ],
            [
                { type: "text", text: "Fun Fact: The skript kiddie the faggot of this site didn't bother checking if the text that goes into the dialog box is HTML code." },
                { type: "html", text: "<img src='./img/misc/topjej.png'></img>", say: "omg its the toppest jej" },
            ],
        ],
